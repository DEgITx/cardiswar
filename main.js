    ...
    return (g | (b << 8) | (r << 16));
	  ...
	  function drawInventory(align, alignLength)
	  {
			...
			player.inventory.forEach(function(card, index){
				...
				if(text.length > 0)
				{
					var cardText = game.add.text(90, 550, text, { fontSize: '31px', fill: '#000', wordWrap: true, wordWrapWidth: 628 });
				...
				card.penalty.forEach(function(penalty, i)
				{
					...
					var penaltyText = game.add.text(135, 70 + 639 + (i*58), (card.currentPenalty == i ? '> ' : '') + 'штраф ' + (i+1) + ": " + penalty, { fontSize: '30px', fill: '#000' });
	  ...
	  function drawCardInfo()
	  {
		  if(cardInfoGroup != null)
			...
			{
				var cardCost = game.add.text(game.world.width - 245, 364, "Эта карта стоит: " + map[player.position].cost, { fontSize: '18px', fill: '#fff' });
				...
				{
					var descText = game.add.text(game.world.width - 245, 390, map[player.position].description, { fontSize: '12px', fill: '#fff', wordWrap: true, wordWrapWidth: 245 });
	  ...
	  function drawMap()
	  {
		...
		promise.then(function(){
			for(var i = 0; i < map.length; i++)
			{
				if(map[i].cost > 0)
				{
					var costText = game.add.text(map[i].x + map[i].width/2 - 23, map[i].y + map[i].height/2 - 10, map[i].cost, { fontSize: '20px', fill: '#000' });
